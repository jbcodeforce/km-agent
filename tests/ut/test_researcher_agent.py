"""Researcher agent wiring — DuckDuckGo search worker."""
...
def test_build_researcher_agent_stateless(monkeypatch, mock_knowledge) -> None:
    ...
    assert "web_search" in agent.instructions
    assert "one URL at a time" in agent.instructions
    assert "parallel_search" not in agent.instructions


def test_build_researcher_agent_without_parallel_key(monkeypatch, mock_knowledge) -> None:
